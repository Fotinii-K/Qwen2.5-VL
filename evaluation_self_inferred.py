...
responses_output_dir = os.path.join(output_root, "model_responses_self_inferred")
...
def process_video(video_path):
    ...
    task_identification = """What is the task and the goal of the task being executed in the video?
                            Consider any visual hints that might provide context about the task.
    ...
    messages = [
        ...
        "content": [
            ...
            {"type": "text", "text": task_identification},
            ],
        }
    ]
    task_def = process_video_inference(messages)
    print(f"Task Definition for {video_name}:\n", task_def)
    messages.append({"role": "assistant", "content": task_def})

    success_prompt = f"""
        Is the task currently being executed successfully completed, according to {task_def}? Explicitly address it, with a yes or no and why.
    ...
    return video_name, task_def, task_success, cause_text, grounding_response, localization_response, response_bbox, selected_frame_path, frame_bbox_path, planning_response
...
def save_response_outputs(
    ...
    add_if_present("output_text_task_identification", task_def)
    ...
    if frame_bbox_path is not None and os.path.exists(frame_bbox_path):
        responses["frame_bbox_path"] = os.path.basename(frame_bbox_path)  
...
for path in video_paths:
    try:
        print(f"\n--- Processing {path} ---")
        video_name, task_def, task_success, cause_text, grounding_response, localization_response, response_bbox, selected_frame_path, frame_bbox_path, planning_response = process_video(path)
